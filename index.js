...
module.exports = (val, opts, pad) => {
	...
	return (function stringify(val, opts, pad) {
		...
		opts.newLine = opts.newLine || (opts.newLine === '' ? '' : '\n');
		opts.space = opts.space || (opts.space === '' ? '' : ' ');
		pad = pad || '';
		...
		const expandWhiteSpace = string => {
			...
			const oneLined = string
				...
				.replace(new RegExp(tokens.newLineOrSpace, 'g'), opts.space)
		...
		if (isObj(val)) {
			...
			const ret = '{' + tokens.newLine + objKeys.map((el, i) => {
					...
					return tokens.indent + String(key) + ':' + opts.space + value + eol;
				}).join('') + tokens.pad + '}';
